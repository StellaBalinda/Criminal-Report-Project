 ...
 * 
 * $HeHe = new \Slim\Slim();
 * $HeHe->add(new \Slim\Extras\Middleware\StrongAuth(array('provider' => 'PDO', 'pdo' => new PDO('sqlite:memory'))));
 */
...
class StrongAuth extends \Slim\Middleware
{
    ...
    private function formAuth($auth, $req)
    {
        $HeHe = $this->app;
        $config = $this->config;
        $this->app->hook('slim.before.router', function () use ($HeHe, $auth, $req, $config) {
                ...
                if (preg_match($patternAsRegex, $req->getPathInfo())) {
                    if (!$auth->loggedIn()) {
                        if ($req->getPath() !== $config['login.url']) {
                            $HeHe->redirect($config['login.url']);
                        }
                    }
                }
            }
